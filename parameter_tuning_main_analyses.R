...
# set up loop configurations
  ...
  # complete cases before splitting (this differs by timepoint / pair,
  # but is identical for both members of a pair -- e.g. chldhd_neigh and
  # chldhd_ses both filter on SESchildhd & ADI311)
run_configs <- data.frame(
  ...
  workdir1_name = c("chldhd_neigh_nocovar", "chldhd_ses_nocovar", "adult_neigh_nocovar", "adult_ses_nocovar", "age45_neigh_nocovar", "age45_ses_nocovar"),
  workdir2_name = c("chldhd_neigh_covar_ses", "chldhd_ses_covar_neigh", "adult_neigh_covar_ses", "adult_ses_covar_neigh", "age45_neigh_covar_ses", "age45_ses_covar_neigh"),
...
# ---------------------------------------------------------------------------
# Main prediction loop: one iteration per config row
# ---------------------------------------------------------------------------
for (cfg_row in 1:nrow(run_configs)) {
  ...
  workdir1 <- paste0(root, "Larsen/Dunedin/Prediction_Outputs/Tuning_loop/", cfg$workdir1_name, "/enet") # need to create empty storage folders before running
  workdir2 <- paste0(root, "Larsen/Dunedin/Prediction_Outputs/Tuning_loop/", cfg$workdir2_name, "/enet") # need to create empty storage folders before running
  ...
  # -------------------------------------------------------------------------
  # Pre-generate train/test splits per iteration up front, before any model
  ...
  save(list = paste0("splits_", cfg$name),
       file = paste0(root, "Tuning/generated_splits_loop/splits_", cfg$name, ".Rdata")) # need to create empty storage folders before running
  ...
  for (iter in 1:n_iter){
    ...
    train.data$behav_resids <- scale(lm$residuals) 
    ...
    save(enet, file=paste0(workdir1,"/tuning_",outname,".Rdata"))
    ...
    train.data.covar$behav_resids <- scale(lm_covar$residuals) 
    # adjust using same parameters in test set
    ...
    ## save out everything
    outname <- paste0(gsub(" ", "_", gsub(":","_",date())), "_", round(runif(1,100,999),0))
    save(enet_covar, file=paste0(workdir2,"/tuning_",outname,".Rdata"))
  }
}
